...
1. **frontend/v1:** Tetris React application (v1) with multi-stage Dockerfile and Nginx configuration.
2. **frontend/v2:** Enhanced Tetris React application (v2) with multi-stage Dockerfile and Nginx configuration.
3. **eks:** Terraform scripts for provisioning the Amazon EKS cluster and VPC.
4. **jenkins:** Jenkins Declarative Pipeline definitions (TetrisV1, TetrisV2, and EKS-Terraform).
5. **jenkins-server:** Terraform scripts for provisioning the Jenkins EC2 server with tools installation.
6. **k8s-manifests:** Kubernetes manifest files (Deployment, Service, and Ingress) for application deployment.
